...
{
    /**
     * Guarda cada agente (WhatsApp + su tienda + su código 4Life + su propia
     * ubicación), y los valores generales (código/tienda/dirección) de
     * respaldo de UNA región. Se llama una vez por cada tarjeta de región
     ...
     * whatsapp_direcciones[] / whatsapp_direcciones_cortas[] / whatsapp_lats[]
     * / whatsapp_lngs[] — todos van en el MISMO índice por fila (el JS del
     * panel los agrupa en la misma fila removible), así quedan correctamente
     * emparejados aunque se borren filas de en medio. La landing rota cuál
     * está "de turno" cada 10 minutos — ver Region::agenteActivo().
     */
    public function update(Request $request, Region $region): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'whatsapp_direcciones'         => ['array'],
            'whatsapp_direcciones.*'       => ['nullable', 'string', 'max:500'],
            'whatsapp_direcciones_cortas'  => ['array'],
            'whatsapp_direcciones_cortas.*' => ['nullable', 'string', 'max:100'],
            'whatsapp_lats'                => ['array'],
            'whatsapp_lats.*'              => ['nullable', 'numeric', 'between:-90,90'],
            'whatsapp_lngs'                => ['array'],
            'whatsapp_lngs.*'              => ['nullable', 'numeric', 'between:-180,180'],
        ...
        $direccionesInput  = $validated['whatsapp_direcciones'] ?? [];
        $direccionCInput   = $validated['whatsapp_direcciones_cortas'] ?? [];
        $latsInput         = $validated['whatsapp_lats'] ?? [];
        $lngsInput         = $validated['whatsapp_lngs'] ?? [];
        ...
        $agentes = collect($numerosInput)
            ->map(fn ($numero, $i) => [
                ...
                'direccion'       => trim((string) ($direccionesInput[$i] ?? '')) ?: null,
                'direccion_corta' => trim((string) ($direccionCInput[$i] ?? '')) ?: null,
                'lat'             => ($latsInput[$i] ?? '') !== '' ? (float) $latsInput[$i] : null,
                'lng'             => ($lngsInput[$i] ?? '') !== '' ? (float) $lngsInput[$i] : null,
            ])
        ...
        unset(
            $validated['whatsapp_numeros'], $validated['whatsapp_tiendas'], $validated['whatsapp_codigos'],
            $validated['whatsapp_direcciones'], $validated['whatsapp_direcciones_cortas'],
            $validated['whatsapp_lats'], $validated['whatsapp_lngs'],
        );
